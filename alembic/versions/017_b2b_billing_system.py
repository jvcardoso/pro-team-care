...
def upgrade() -> None:
    ...
    op.create_table('company_subscriptions',
        ...
        sa.Column('updated_at', sa.DateTime(), nullable=True),
        sa.PrimaryKeyConstraint('id'),
        sa.ForeignKeyConstraint(['company_id'], ['master.companies.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['plan_id'], ['master.subscription_plans.id'], ),
        schema='master'
    )

    # Create Pro Team Care invoices table
    op.create_table('proteamcare_invoices',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('company_id', sa.Integer(), nullable=False),
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.ForeignKeyConstraint(['company_id'], ['master.companies.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['subscription_id'], ['master.company_subscriptions.id'], ondelete='CASCADE'),
        sa.UniqueConstraint('invoice_number'),
        schema='master'
    )

    # Create indexes for better performance
    op.create_index('idx_company_subscriptions_company_id', 'company_subscriptions', ['company_id'], schema='master')
    op.create_index('idx_company_subscriptions_status', 'company_subscriptions', ['status'], schema='master')
    op.create_index('idx_proteamcare_invoices_company_id', 'proteamcare_invoices', ['company_id'], schema='master')
    op.create_index('idx_proteamcare_invoices_status', 'proteamcare_invoices', ['status'], schema='master')
    op.create_index('idx_proteamcare_invoices_due_date', 'proteamcare_invoices', ['due_date'], schema='master')
...
def downgrade() -> None:
    # Drop indexes
    op.drop_index('idx_proteamcare_invoices_due_date', 'proteamcare_invoices', schema='master')
    op.drop_index('idx_proteamcare_invoices_status', 'proteamcare_invoices', schema='master')
    op.drop_index('idx_proteamcare_invoices_company_id', 'proteamcare_invoices', schema='master')
    op.drop_index('idx_company_subscriptions_status', 'company_subscriptions', schema='master')
    op.drop_index('idx_company_subscriptions_company_id', 'company_subscriptions', schema='master')
