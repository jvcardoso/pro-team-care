...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "report_type", sa.String(30), nullable=False
        ...
        sa.Column(
            "report_month", sa.Integer(), nullable=True
        ),  # NULL para relatórios anuais
        sa.Column("generated_at", sa.DateTime(), nullable=False),
        sa.Column(
            "report_data", sa.JSON(), nullable=False
        ),  # Dados completos do relatório
        sa.Column(
            "file_path", sa.String(500)
        ),  # Caminho para arquivo PDF/Excel se gerado
        sa.Column(
            "email_sent", sa.Boolean(), default=False
        ),  # Flag se foi enviado por email
        sa.Column("email_sent_at", sa.DateTime()),
        sa.Column("recipients", sa.JSON()),  # Lista de destinatários do email
        sa.Column(
            "status", sa.String(20), default="generated"
        ...
        sa.CheckConstraint(
            "report_type IN ('monthly_contract', 'monthly_company', 'annual_summary', 'custom')",
            name="automated_reports_type_check",
        ),
        sa.CheckConstraint(
            "status IN ('generated', 'sent', 'archived', 'failed')",
            name="automated_reports_status_check",
        ),
        sa.CheckConstraint(
            "report_month IS NULL OR (report_month >= 1 AND report_month <= 12)",
            name="automated_reports_month_check",
        ),
        schema="master",
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "schedule_type", sa.String(20), nullable=False
        ),  # monthly, quarterly, annual
        sa.Column(
            "company_id", sa.BigInteger(), nullable=True
        ),  # NULL = todos as empresas
        sa.Column(
            "contract_id", sa.BigInteger(), nullable=True
        ),  # NULL = todos os contratos da empresa
        sa.Column(
            "report_types", sa.JSON(), nullable=False
        ),  # Tipos de relatório a gerar
        sa.Column(
            "recipients", sa.JSON(), nullable=False
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "schedule_type IN ('monthly', 'quarterly', 'annual')",
            name="report_schedules_type_check",
        ),
        sa.CheckConstraint(
            "execution_day IS NULL OR (execution_day >= 1 AND execution_day <= 31)",
            name="report_schedules_day_check",
        ),
        sa.CheckConstraint(
            "execution_hour >= 0 AND execution_hour <= 23",
            name="report_schedules_hour_check",
        ),
        schema="master",
    )
    ...
    op.create_table(
        ...
        sa.Column("id", sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column(
            "schedule_id", sa.BigInteger(), nullable=True
        ),  # NULL para execuções manuais
        sa.Column(
            "execution_type", sa.String(20), nullable=False
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "execution_type IN ('scheduled', 'manual', 'api')",
            name="report_execution_log_type_check",
        ),
        sa.CheckConstraint(
            "status IN ('running', 'completed', 'failed', 'cancelled')",
            name="report_execution_log_status_check",
        ),
        schema="master",
    )
    ...
    op.create_index(
        "automated_reports_company_date_idx",
        "automated_reports",
        ["company_id", "report_year", "report_month"],
        schema="master",
    )
    op.create_index(
        "automated_reports_contract_date_idx",
        "automated_reports",
        ["contract_id", "report_year", "report_month"],
        schema="master",
    )
    op.create_index(
        "automated_reports_type_date_idx",
        "automated_reports",
        ["report_type", "report_year", "report_month"],
        schema="master",
    )
    op.create_index(
        "automated_reports_generated_at_idx",
        "automated_reports",
        ["generated_at"],
        schema="master",
    )
    op.create_index(
        "automated_reports_status_idx", "automated_reports", ["status"], schema="master"
    )

    op.create_index(
        "report_schedules_next_execution_idx",
        "report_schedules",
        ["next_execution", "is_active"],
        schema="master",
    )
    op.create_index(
        "report_schedules_company_idx",
        "report_schedules",
        ["company_id"],
        schema="master",
    )
    ...
    op.create_index(
        "report_execution_log_started_at_idx",
        "report_execution_log",
        ["started_at"],
        schema="master",
    )
    op.create_index(
        "report_execution_log_status_idx",
        "report_execution_log",
        ["status"],
        schema="master",
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP TRIGGER IF EXISTS trigger_update_report_schedule_next_execution ON master.report_schedules;"
    )
    op.execute(
        "DROP FUNCTION IF EXISTS master.update_report_schedule_next_execution();"
    )
    op.execute(
        "DROP FUNCTION IF EXISTS master.get_pending_scheduled_reports(TIMESTAMP);"
    )
    ...
    op.drop_index(
        "report_schedules_company_idx", table_name="report_schedules", schema="master"
    )
    op.drop_index(
        "report_schedules_next_execution_idx",
    ...
    op.drop_index(
        "automated_reports_status_idx", table_name="automated_reports", schema="master"
    )
    op.drop_index(
        "automated_reports_generated_at_idx",
        table_name="automated_reports",
        schema="master",
    )
    op.drop_index(
        "automated_reports_type_date_idx",
