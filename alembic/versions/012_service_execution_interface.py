...
def upgrade() -> None:
    ...
    op.create_table('service_executions',
        ...
        sa.Column('location_type', sa.String(20), nullable=False),  # home, clinic, hospital, telemedicine
        sa.Column('address_id', sa.BigInteger()),
        sa.Column('coordinates', sa.String(50)),  # GPS coordinates
        sa.Column('status', sa.String(20), nullable=False, default='scheduled'),  # scheduled, in_progress, completed, cancelled, no_show
        ...
        sa.Column('billing_status', sa.String(20), default='pending'),  # pending, approved, rejected, paid
        ...
        sa.ForeignKeyConstraint(['patient_id'], ['master.users.id'], ),
        sa.ForeignKeyConstraint(['service_id'], ['master.services_catalog.id'], ),
        sa.ForeignKeyConstraint(['address_id'], ['master.addresses.id'], ),
        ...
        sa.CheckConstraint("status IN ('scheduled', 'in_progress', 'completed', 'cancelled', 'no_show')", name='service_executions_status_check'),
        sa.CheckConstraint("location_type IN ('home', 'clinic', 'hospital', 'telemedicine')", name='service_executions_location_type_check'),
        sa.CheckConstraint("billing_status IN ('pending', 'approved', 'rejected', 'paid')", name='service_executions_billing_status_check'),
        sa.CheckConstraint("satisfaction_rating IS NULL OR (satisfaction_rating >= 1 AND satisfaction_rating <= 5)", name='service_executions_satisfaction_rating_check'),
        sa.CheckConstraint("sessions_consumed > 0", name='service_executions_sessions_consumed_check'),
        schema='master'
    )
    ...
    op.create_table('professional_schedules',
        ...
        sa.ForeignKeyConstraint(['updated_by'], ['master.users.id'], ),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('professional_id', 'date', 'start_time', name='professional_schedules_unique'),
        sa.CheckConstraint("location_preference IN ('home', 'clinic', 'any')", name='professional_schedules_location_preference_check'),
        sa.CheckConstraint("end_time > start_time", name='professional_schedules_time_check'),
        schema='master'
    )

    # Create execution checklists table
    op.create_table('execution_checklists',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column('execution_id', sa.BigInteger(), nullable=False),
        sa.Column('checklist_type', sa.String(30), nullable=False),  # pre_execution, during_execution, post_execution
        ...
        sa.Column('notes', sa.Text()),
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('NOW()')),
        sa.ForeignKeyConstraint(['execution_id'], ['master.service_executions.id'], ),
        sa.ForeignKeyConstraint(['completed_by'], ['master.users.id'], ),
        sa.PrimaryKeyConstraint('id'),
        sa.CheckConstraint("checklist_type IN ('pre_execution', 'during_execution', 'post_execution')", name='execution_checklists_type_check'),
        schema='master'
    )

    # Create indexes for performance
    op.create_index('service_executions_authorization_idx', 'service_executions', ['authorization_id'], schema='master')
    op.create_index('service_executions_professional_idx', 'service_executions', ['professional_id'], schema='master')
    op.create_index('service_executions_patient_idx', 'service_executions', ['patient_id'], schema='master')
    ...
    op.create_index('service_executions_billing_status_idx', 'service_executions', ['billing_status'], schema='master')

    op.create_index('professional_schedules_professional_date_idx', 'professional_schedules', ['professional_id', 'date'], schema='master')
    op.create_index('professional_schedules_date_available_idx', 'professional_schedules', ['date', 'is_available'], schema='master')

    op.create_index('execution_checklists_execution_idx', 'execution_checklists', ['execution_id'], schema='master')
    op.create_index('execution_checklists_type_idx', 'execution_checklists', ['checklist_type'], schema='master')
...
def downgrade() -> None:
    ...
    op.execute("DROP TRIGGER IF EXISTS trigger_generate_execution_code ON master.service_executions;")
    op.execute("DROP FUNCTION IF EXISTS master.generate_execution_code();")
    op.execute("DROP FUNCTION IF EXISTS master.validate_execution_scheduling(BIGINT, DATE, TIME, TIME, BIGINT);")

    # Drop indexes
    op.drop_index('execution_checklists_type_idx', table_name='execution_checklists', schema='master')
    op.drop_index('execution_checklists_execution_idx', table_name='execution_checklists', schema='master')
    op.drop_index('professional_schedules_date_available_idx', table_name='professional_schedules', schema='master')
    op.drop_index('professional_schedules_professional_date_idx', table_name='professional_schedules', schema='master')
    op.drop_index('service_executions_billing_status_idx', table_name='service_executions', schema='master')
    op.drop_index('service_executions_status_idx', table_name='service_executions', schema='master')
    op.drop_index('service_executions_execution_date_idx', table_name='service_executions', schema='master')
    op.drop_index('service_executions_patient_idx', table_name='service_executions', schema='master')
    op.drop_index('service_executions_professional_idx', table_name='service_executions', schema='master')
    op.drop_index('service_executions_authorization_idx', table_name='service_executions', schema='master')
