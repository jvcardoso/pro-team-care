...
ADD COLUMN IF NOT EXISTS establishment_id BIGINT REFERENCES master.establishments(id),
ADD COLUMN IF NOT EXISTS context_type VARCHAR(20) DEFAULT 'establishment' CHECK (context_type IN ('company', 'establishment', 'client')),
ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active', 'inactive', 'pending', 'suspended', 'blocked')),
...
CREATE INDEX IF NOT EXISTS users_establishment_id_idx ON master.users(establishment_id);
CREATE INDEX IF NOT EXISTS users_context_type_idx ON master.users(context_type);
CREATE INDEX IF NOT EXISTS users_status_idx ON master.users(status);
CREATE INDEX IF NOT EXISTS users_activation_token_idx ON master.users(activation_token);
CREATE INDEX IF NOT EXISTS users_password_reset_token_idx ON master.users(password_reset_token);
CREATE INDEX IF NOT EXISTS users_invited_by_idx ON master.users(invited_by_user_id);
...
ADD CONSTRAINT users_context_consistency CHECK (
    (context_type = 'company' AND establishment_id IS NULL) OR
    (context_type = 'establishment' AND establishment_id IS NOT NULL) OR
    (context_type = 'client')
);
