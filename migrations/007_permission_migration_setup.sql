...
('consultor', 'Consultor Externo', 'Acesso limitado para consultoria', 30, 49, 'establishment',
...
CREATE INDEX IF NOT EXISTS permission_audit_user_idx ON master.permission_audit_log(user_id);
CREATE INDEX IF NOT EXISTS permission_audit_permission_idx ON master.permission_audit_log(permission);
CREATE INDEX IF NOT EXISTS permission_audit_date_idx ON master.permission_audit_log(created_at);
...
SELECT
    ...
    master.suggest_role_template_for_level(r.level, ur.context_type) as suggested_template,
    master.get_permissions_for_level(r.level, ur.context_type) as equivalent_permissions,
...
ORDER BY u.name, ur.context_type, r.level DESC;
