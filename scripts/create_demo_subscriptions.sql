...
WITH subscription AS (
    SELECT id FROM master.company_subscriptions WHERE company_id = 59
)
...
WITH subscription AS (
    SELECT id FROM master.company_subscriptions WHERE company_id = 60
)
...
WITH subscription AS (
    SELECT id FROM master.company_subscriptions WHERE company_id = 61
)
...
FROM master.subscription_plans AS sp
LEFT JOIN master.company_subscriptions AS cs ON sp.id = cs.plan_id
...
INNER JOIN master.subscription_plans AS sp ON cs.plan_id = sp.id
ORDER BY sp.monthly_price DESC, p.name ASC;
