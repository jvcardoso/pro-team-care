...
async def move_contract_menus_to_business():
    ...
    try:
        async for db in get_db():
            ...
            contract_menu_ids = [142, 146, 148, 151]  # Gestão de Contratos, Gestão de Vidas, Catálogo de Serviços, Relatórios Contratos

            print(f"📋 Movendo 4 menus de contratos do Home Care (ID: {home_care_id}) para Negócio (ID: {business_id})")
            ...
            result = await db.execute(update_contract_menus_query, {
                "business_id": business_id,
                "menu_ids": contract_menu_ids
            })
            ...
            home_care_verify_query = text("""
                ...
                ORDER BY sort_order
            """)
            result = await db.execute(home_care_verify_query, {"home_care_id": home_care_id})
            ...
            """)
            result = await db.execute(business_verify_query, {"business_id": business_id})
