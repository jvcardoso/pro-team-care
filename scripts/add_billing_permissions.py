    ...
    async with async_session() as db:
        try:
            ...
            for perm in billing_permissions:
                ...
                logger.info(f"Added permission: {perm['name']} - {perm['display_name']}")
...
async def assign_billing_permissions_to_roles(db):
    ...
    role_permissions = {
        'Administrador Geral': [
            ...
            'billing_approve', 'billing_admin', 'billing_reports', 'billing_dashboard'
        ],
        ...
        'Gestor Financeiro': [
            'billing_view', 'billing_create', 'billing_update',
            'billing_approve', 'billing_reports', 'billing_dashboard'
        ],
        'Supervisor': [
            'billing_view', 'billing_update', 'billing_dashboard'
        ],
        'Operador': [
            'billing_view'
        ],
    ...
    try:
        for role_name, permissions in role_permissions.items():
            ...
            for perm_code in permissions:
                # Get permission ID
                perm_query = text("SELECT id FROM master.permissions WHERE name = :name")
