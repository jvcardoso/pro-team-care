...
class TechnicalDebtAuditor:
    ...
    def find_python_files(self) -> None:
        ...
        exclude_dirs = {'.git', '__pycache__', 'node_modules', '.pytest_cache', 'coverage', 'e2e', 'venv'}
            ...
            try:
                ...
                for result in results:
                    ...
                    file_complexity['functions'].append({
                        ...
                        'line': result.lineno,
                        'type': getattr(result, 'type', 'unknown')  # Handle missing type attribute
                    })
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if clean_line and not clean_line.startswith('#'):
                        ...
                        code_lines[clean_line].append((str(file_path.relative_to(self.project_root)), i))
        ...
        for file_path in self.python_files:
            try:
                    ...
                    if isinstance(node, (ast.FunctionDef, ast.ClassDef)):
                        ...
                        if len(block_code) > 100:  # Blocos significativos
                            blocks.append({
                                ...
                                'hash': hashlib.md5(block_code.encode()).hexdigest()
                            })
                ...
                if duplicates_in_file:
                    internal_duplicates[str(file_path.relative_to(self.project_root))] = duplicates_in_file
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"Arquivos com análise de complexidade: {len(self.complexity_report)}")
        report.append(f"Duplicatas entre arquivos encontradas: {len(self.duplicate_report)}")
        report.append("")
        ...
        if self.complexity_report:
            ...
            for file_path, data in self.complexity_report.items():
                ...
                report.append(f"  Funções de alta complexidade (>10): {data['high_complexity_count']}")
                ...
                if high_complexity:
                    ...
                    for func in high_complexity[:5]:  # Top 5
                        report.append(f"    - {func['name']} (complexidade: {func['complexity']}, linha: {func['line']})")
                report.append("")
            ...
            report.append(f"Complexidade média por função: {total_complexity / total_functions:.1f}" if total_functions > 0 else "N/A")
            report.append("")
        else:
            report.append("⚠️  Análise de complexidade não disponível (radon não instalado)")
            report.append("")
        ...
        if self.duplicate_report:
            ...
            for key, data in list(self.duplicate_report.items())[:20]:  # Top 20
                report.append(f"🔄 Linha duplicada ({data['occurrences']} ocorrências):")
                report.append(f"  📝 {data['line']}")
                report.append(f"  📁 Arquivos: {', '.join(data['files'])}")
                report.append("")

            report.append(f"📊 Total de duplicatas encontradas: {len(self.duplicate_report)}")
            report.append("")
        else:
            report.append("✅ Nenhuma duplicata significativa encontrada entre arquivos")
            report.append("")
            ...
            for file_name, duplicates in internal_duplicates.items():
                ...
                for dup in duplicates[:3]:  # Top 3 por arquivo
                    ...
                    for block in dup['blocks'][:2]:  # Mostrar 2 exemplos
                        report.append(f"    - {block['name']} (linha {block['start_line']})")
                report.append("")
        ...
        if high_complexity_functions > 0:
            ...
            recommendations.append("   - Quebrar funções com complexidade > 10 em funções menores")
            recommendations.append("   - Aplicar padrão Strategy para lógica condicional complexa")
            recommendations.append("   - Extrair métodos privados para reduzir complexidade")
        ...
        if self.duplicate_report:
            ...
            recommendations.append("   - Usar herança ou composição para evitar duplicação")
        ...
        report.append("2. Executar análise completa: python scripts/audit_technical_debt.py")
