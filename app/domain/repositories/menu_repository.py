...
class MenuRepository:
    ...
    async def get_user_menus(
        ...
        try:
            ...
            if is_root:
                ...
                if not has_crud_menu:
                    ...
                    if admin_menu_id:
                        ...
                        logger.info(
                            "Menu CRUD injetado para usuário ROOT",
                            user_id=user_id,
            ...
            logger.error("Erro ao buscar menus", user_id=user_id, error=str(e))
            raise
    ...
    async def get_user_info(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                "Erro ao buscar informações do usuário", user_id=user_id, error=str(e)
            )
            return None

    async def get_context_info(
        self, context_type: str, context_id: Optional[int]
    ) -> Dict[str, Any]:
        """
        ...
        if context_type == "system":
            ...
            query = text(
                """
                SELECT c.id, p.name, p.trade_name
            ...
            try:
                ...
                if row:
                    return {
                        "type": "company",
                        "id": row[0],
                        "name": row[1],  # name from people table
                        "trade_name": row[2],
                        "description": f"Empresa {row[1]}",
                    }
            except Exception as e:
                logger.error(
                    "Erro ao buscar empresa", context_id=context_id, error=str(e)
                )
            ...
            query = text(
                ...
                WHERE e.id = :id AND e.deleted_at IS NULL AND p.deleted_at IS NULL AND cp.deleted_at IS NULL
            """
            )
    ...
    async def log_menu_access(
        ...
        if is_root:
            try:
                ...
                logger.info(
                    "ROOT menu access (audit log disabled)",
                    user_id=user_id,
                ...
                logger.error("Erro ao logar acesso ROOT", user_id=user_id, error=str(e))
