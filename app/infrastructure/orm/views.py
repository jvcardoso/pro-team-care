...
from sqlalchemy import (
    Column, BigInteger, String, Boolean, DateTime, Integer, Text, JSON
)
...
class UserHierarchicalView(Base):
    ...
    # Campos hierárquicos calculados
    can_manage_users = Column(Boolean)
    can_access_company = Column(Boolean)
    can_access_establishment = Column(Boolean)
    hierarchy_path = Column(String)
...
class MenuHierarchyView(Base):
    ...
    # Hierarquia calculada
    full_path_name = Column(String)
    breadcrumb_path = Column(String)
...
class UserAdminView(Base):
    ...
    user_is_active = Column(Boolean)
    user_is_system_admin = Column(Boolean)
    user_has_two_factor = Column(Boolean)
