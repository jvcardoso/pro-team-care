...
class AddressEnrichmentService:
    ...
    async def enrich_address(self, address_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if enriched_data.get("is_validated") and enriched_data.get("enrichment_source"):
            logger.info(
                "Address already enriched, skipping",
                ...
                enriched_data.update(
                    {
                        ...
                        "details": viacep_data.get("complemento")
                        or address_data.get("details", ""),
                        # Códigos oficiais do ViaCEP
                        "ibge_city_code": (
                            int(viacep_data["ibge"])
                            if viacep_data.get("ibge")
                            else address_data.get("ibge_city_code")
                        ),
                        "gia_code": viacep_data.get("gia")
                        or address_data.get("gia_code"),
                        "siafi_code": viacep_data.get("siafi")
                        or address_data.get("siafi_code"),
                        "area_code": viacep_data.get("ddd")
                        or address_data.get("area_code"),
                        "ibge_state_code": self.get_ibge_state_code(
                            viacep_data.get("uf")
                        )
                        or address_data.get("ibge_state_code"),
        ...
        if not enriched_data.get("latitude") or not enriched_data.get("longitude"):
            ...
            for address_variation in address_variations:
                ...
                clean_address = ", ".join(
                    [
                        part.strip()
                        for part in address_variation.split(",")
                        if part.strip()
                    ]
                )
                ...
                if len(clean_address) > 10:  # Endereço mínimo válido
                    ...
                    if (
                        geo_data
                        and geo_data.get("latitude")
                        and geo_data.get("longitude")
                    ):
                        enriched_data.update(
                            {
                                "latitude": geo_data["latitude"],
                                "longitude": geo_data["longitude"],
                                "geocoding_accuracy": geo_data.get(
                                    "geocoding_accuracy"
                                ),
