...
class BillingSchedulerService:
    ...
    async def schedule_automatic_billing(
        ...
        if self._is_similar_job_running("auto_billing", billing_date):
            raise ValueError(f"Automatic billing job for {billing_date} is already running")
    ...
    async def schedule_recurrent_billing(self) -> str:
        ...
        # Create and start the job
        task = asyncio.create_task(
            self._execute_recurrent_billing_job(job_id=job_id)
        )
    ...
    async def schedule_fallback_processing(self, days_back: int = 7) -> str:
        ...
        logger.info(
            "Scheduling fallback processing job",
            job_id=job_id,
            days_back=days_back
        )
        ...
        # Create and start the job
        task = asyncio.create_task(
            self._execute_fallback_processing_job(
                job_id=job_id,
                days_back=days_back
            )
        )
    ...
    async def schedule_invoice_status_sync(self) -> str:
        ...
        # Create and start the job
        task = asyncio.create_task(
            self._execute_status_sync_job(job_id=job_id)
        )
    ...
    async def _execute_automatic_billing_job(
        ...
        try:
            async with get_db() as db:
                ...
                job_result.metrics = {
                    "total_schedules_processed": result.get("total_schedules_processed", 0),
    ...
    async def _execute_fallback_processing_job(
        ...
        try:
            async with get_db() as db:
                ...
                logger.info(
                    "Starting fallback processing",
                    job_id=job_id,
                    days_back=days_back
                )

                # Get failed recurrent billings
                failed_schedules = await billing_repository.get_failed_recurrent_billings(
                    days_back=days_back
                )
                ...
                for schedule in failed_schedules:
                    try:
                        ...
                        failure_result = await billing_service.process_recurrent_billing_failure(
                            schedule_id=schedule.id,
                            error_details={"reason": "scheduled_fallback_processing"}
                        )
    ...
    def _is_similar_job_running(self, job_type: str, context_date: Optional[date] = None) -> bool:
        """Check if a similar job is already running"""
        for job_id, task in self.running_jobs.items():
            if not task.done():
                if job_type == "auto_billing" and context_date:
                    if job_id.startswith(f"auto_billing_{context_date.strftime('%Y%m%d')}"):
                        return True
    ...
    def get_scheduler_metrics(self) -> Dict[str, Any]:
        ...
        completed_jobs = len([r for r in self.job_results if r.status == JobStatus.COMPLETED])
