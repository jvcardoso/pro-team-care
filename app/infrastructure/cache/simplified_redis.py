...
class SimplifiedRedisClient:
    ...
    async def connect(self):
        """Connect to Redis server with retry logic - fails if connection cannot be established"""
        last_error = None
        ...
        for attempt in range(self._connection_retries):
            try:
                ...
                if attempt < self._connection_retries - 1:
                    logger.warning(
                        f"Redis connection attempt {attempt + 1} failed: {e}. Retrying in {self._retry_delay}s..."
                    )
                    await asyncio.sleep(self._retry_delay)
                    continue
                else:
                    logger.error(
                        f"Redis connection failed after {self._connection_retries} attempts: {e}"
                    )
        ...
        logger.error(
            f"Redis connection failed after {self._connection_retries} attempts. Last error: {last_error}"
        )
        raise ConnectionError(
            f"Cannot connect to Redis server. Last error: {last_error}. System requires Redis connection to function."
        )
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Cache set: {key} ({method.value}, {len(serialized_value)} bytes, TTL: {ttl}s)"
            )
            return True
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        for method in methods:
            try:
                ...
                logger.debug(
                    f"Cache get attempt failed for {key} with {method.value}: {e}"
                )
                continue
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            keys_to_delete = [
                self._make_key(key, method) for method in SerializationMethod
            ]
    ...
    async def exists(self, key: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Redis connection error during exists check for key {key}: {e}"
            )
            return False
        except Exception as e:
            logger.error(
                f"Unexpected error during cache exists check for key {key}: {e}"
            )
            return False
        ...
        try:
            ...
            for method in SerializationMethod:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error scanning for pattern {prefixed_pattern}: {e}"
                    )
                    continue
                ...
                if keys:
                    try:
                        ...
                        logger.error(
                            f"Error deleting keys for pattern {prefixed_pattern}: {e}"
                        )
                        continue
            ...
            logger.error(
                f"Redis connection error during clear pattern for {pattern}: {e}"
            )
            return 0
        except Exception as e:
