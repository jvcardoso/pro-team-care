...
class MenuCacheService:
    ...
    async def cache_menu_tree(
        ...
        try:
            ...
            success = await redis.set(
                cache_key, cache_data, self.ttl_config["menu_tree"]
            )
    ...
    async def get_menu_tree(
        ...
        try:
            ...
            logger.error(
                "Error retrieving menu tree from cache", error=str(e), user_id=user_id
            )
            return None

    async def cache_menu_list(
        self,
        ...
        try:
            cache_key = self._make_cache_key(
                "list", skip, limit, parent_id, status, search, level
            )
            ...
            success = await redis.set(
                cache_key, cache_data, self.ttl_config["menu_list"]
            )
    ...
    async def cache_menu_item(self, menu: MenuEntity) -> bool:
        ...
        try:
            ...
            success = await redis.set(
                cache_key, serialized_menu, self.ttl_config["menu_item"]
            )

            if success:
                logger.debug(
                    "Menu item cached", key=cache_key, menu_id=menu.id, name=menu.name
                )
    ...
    async def get_menu_item(self, menu_id: int) -> Optional[MenuEntity]:
        ...
        try:
            ...
            logger.error(
                "Error retrieving menu item from cache", error=str(e), menu_id=menu_id
            )
            return None

    async def cache_user_menus(
        self,
        menus: List[MenuEntity],
        user_id: int,
        context_type: str = "system",
        ...
        try:
            ...
            success = await redis.set(
                cache_key, cache_data, self.ttl_config["user_menus"]
            )
    ...
    async def get_user_menus(
        ...
        try:
            ...
            logger.error(
                "Error retrieving user menus from cache", error=str(e), user_id=user_id
            )
            return None
    ...
    async def cache_search_results(
        ...
        try:
            ...
            cache_key = self._make_cache_key(
                "search", query_hash, user_id, context_type, limit
            )
            ...
            success = await redis.set(
                cache_key, cache_data, self.ttl_config["search_results"]
            )
    ...
    async def get_search_results(
        ...
        try:
            ...
            logger.error(
                "Error retrieving search results from cache", error=str(e), query=query
            )
            return None
    ...
    async def invalidate_menu_caches(self, menu_id: Optional[int] = None) -> int:
        ...
        try:
            ...
            logger.error(
                "Error invalidating menu caches", error=str(e), menu_id=menu_id
            )
            return 0
    ...
    async def warm_cache(
        ...
        logger.info(
            "Cache warming completed", results=results, total_menus=len(all_menus)
        )
