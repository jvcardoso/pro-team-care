...
from app.infrastructure.cache.simplified_redis import simplified_redis_client as redis_client
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            if skip_cache and await skip_cache(*args, **kwargs):
                logger.debug(f"Skipping cache for {func.__name__} due to skip_cache condition")
        ...
        wrapper.cache_key = lambda *args, **kwargs: generate_cache_key(
            f"{key_prefix}:{func.__module__}.{func.__name__}" if key_prefix else f"{func.__module__}.{func.__name__}",
            *args, **kwargs
        )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for pattern in patterns:
                ...
                logger.debug(f"Invalidated {cleared_count} cache entries for pattern: {pattern}")
