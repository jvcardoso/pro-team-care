...
class EnhancedUserRepository(UserRepository):
    ...
    async def get_user_complete_secure(
        ...
        try:
            ...
            self.logger.error(
                "Error fetching secure user data", user_id=user_id, error=str(e)
            )
            raise
    ...
    async def list_users_with_context(
        ...
        try:
            ...
            if search:
                conditions.append(
                    "(person_name ILIKE :search OR user_email ILIKE :search)"
                )
            ...
            users = [
                dict(zip(columns, [getattr(row, col) for col in columns]))
                for row in rows
            ]
    ...
    async def audit_user_access(self, user_id: int, action: str, context: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                "Failed to log audit", user_id=user_id, action=action, error=str(e)
            )
