...
class HierarchicalUserRepository(UserRepository):
    ...
    async def get_user_with_hierarchical_control(
        ...
        try:
            ...
            access_level = await self._get_access_level(requesting_user_id, target_user_id)
            
            if access_level == AccessLevel.NONE:
                self.logger.warning("Access denied", requesting_user_id=requesting_user_id, target_user_id=target_user_id)
                return None
            ...
            self.logger.error("Error in hierarchical user access", 
    ...
    async def list_accessible_users(
        ...
        try:
            ...
            # Aplicar filtros
            if search:
                query_conditions.append("(vc.person_name ILIKE :search OR vc.user_email ILIKE :search)")
            ...
            if hierarchy_filter:
                hierarchy_users = [uid for uid, access in accessible_users.items() 
            ...
            for row in rows:
                ...
                if access_info:
                    user_data = await self._apply_data_masking(user_data, access_info.access_level)
                    user_data['access_reason'] = access_info.reason
                
                users.append(user_data)
            
            # 6. Log da operação para auditoria
            await self._log_user_list_access(requesting_user_id, len(users), search, role_filter)
            
            return users, total
            
        except Exception as e:
            self.logger.error("Error listing accessible users", 
                            requesting_user_id=requesting_user_id, 
                            error=str(e))
            raise

    async def _get_access_level(self, requesting_user_id: int, target_user_id: int) -> AccessLevel:
        """Determina o nível de acesso hierárquico"""
        try:
            # Usar função SQL para verificar acesso hierárquico
            result = await self.db.execute(
                text("SELECT access_level, reason FROM master.get_accessible_users_hierarchical(:user_id) WHERE accessible_user_id = :target_id"),
    ...
    async def _get_accessible_user_ids(self, requesting_user_id: int) -> Dict[int, Any]:
        ...
        try:
            result = await self.db.execute(
                text("SELECT accessible_user_id, access_level, reason FROM master.get_accessible_users_hierarchical(:user_id)"),
                {"user_id": requesting_user_id}
            )
            
            accessible_users = {}
            for row in result.fetchall():
                accessible_users[row.accessible_user_id] = type('AccessInfo', (), {
    ...
    async def _apply_data_masking(self, user_data: Dict[str, Any], access_level: str) -> Dict[str, Any]:
        ...
        if access_level == 'full':
            ...
            sensitive_fields = ['person_tax_id', 'person_birth_date', 'user_preferences']
            ...
            basic_fields = ['user_id', 'user_email', 'person_name', 'establishment_code', 'role_display_name']
            user_data = {k: v for k, v in user_data.items() if k in basic_fields}
        
        return user_data

    async def _log_user_list_access(self, user_id: int, count: int, search: str, role_filter: str):
    ...
    async def get_user_hierarchy_info(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("Error getting user hierarchy info", user_id=user_id, error=str(e))
            raise