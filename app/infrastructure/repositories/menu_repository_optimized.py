...
from sqlalchemy import select, text, and_, or_, func, delete, update
...
from app.infrastructure.cache.menu_cache_service import MenuCacheService, get_menu_cache_service
...
class MenuRepositoryOptimized(MenuRepositoryInterface):
    ...
    def __init__(self, db: AsyncSession, cache_service: Optional[MenuCacheService] = None):
        self.db = db
    ...
    def _entity_to_orm(self, entity: MenuEntity, orm: Optional[MenuORM] = None) -> MenuORM:
    ...
    @timing_decorator
    async def get_by_slug(self, slug: str, parent_id: Optional[int] = None) -> Optional[MenuEntity]:
    ...
    async def get_all(self, 
                     ...
                     level: Optional[int] = None) -> List[MenuEntity]:
        ...
        self.logger.info("Menus listados", 
                        count=len(entities),
    ...
    async def update(self, menu_id: int, menu: MenuEntity) -> Optional[MenuEntity]:
        ...
        if menu.parent_id and not await self.validate_hierarchy_change(menu_id, menu.parent_id):
    ...
    async def get_hierarchy_tree(self, 
                               ...
                               include_inactive: bool = False) -> List[MenuEntity]:
        ...
        self.logger.info("Hierarquia de menus carregada", 
    ...
    async def reorder_siblings(self, parent_id: Optional[int], menu_orders: List[Dict]) -> bool:
        ...
        try:
            ...
            self.logger.info("Menus reordenados", 
                           parent_id=parent_id, 
    ...
    async def validate_hierarchy_change(self, menu_id: int, new_parent_id: Optional[int]) -> bool:
    ...
    async def get_children(self, parent_id: int, recursive: bool = False) -> List[MenuEntity]:
    ...
    async def get_user_accessible_menus(self, 
                                       user_id: int,
                                       context_type: str = "system",
                                       context_id: Optional[int] = None) -> List[MenuEntity]:
        """Buscar menus acessíveis para usuário específico"""
        
        # Tentar cache primeiro
        cache_service = await self._get_cache_service()
        cached_menus = await cache_service.get_user_menus(user_id, context_type, context_id)
        if cached_menus is not None:
            return cached_menus
        ...
        # Cachear resultado
        await cache_service.cache_user_menus(flat_menus, user_id, context_type, context_id)
    ...
    async def search_menus(self, 
                          query: str,
                          user_id: Optional[int] = None,
                          context_type: str = "system",
                          limit: int = 50) -> List[MenuEntity]:
        ...
        # Cachear resultado
        await cache_service.cache_search_results(entities, query, user_id, context_type, limit)
    ...
    async def get_menu_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_children_per_menu': (row.total_menus - row.root_menus) / max(row.root_menus, 1)
        }
    ...
    async def bulk_update_status(self, menu_ids: List[int], status: str) -> bool:
        ...
        try:
            ...
            self.logger.info("Status de menus atualizado em lote", 
                           menu_ids=menu_ids, 
                           new_status=status,
    ...
    async def get_recently_modified(self, 
                                   limit: int = 10,
                                   user_id: Optional[int] = None) -> List[MenuEntity]:
