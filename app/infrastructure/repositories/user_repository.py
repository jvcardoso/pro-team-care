...
# ORM Models
from app.infrastructure.orm.models import People, Phone, Email, Address
...
from app.presentation.schemas.user import (
    ...
    PersonCreatePF, PhoneCreate, EmailCreate, AddressCreate
)
...
class UserRepository(UserRepositoryInterface):
    ...
    async def create(self, user_data: UserCreate) -> UserDetailed:
        ...
        try:
            ...
            existing_user = await self.db.execute(
                select(UserEntity).where(UserEntity.email_address == user_data.email_address)
            )
            ...
            self.logger.info("User created successfully", user_id=user.id, email=user_data.email_address)
            return created_user
    ...
    async def _create_user_contacts(self, user_id: int, person_id: int, user_data: UserCreate):
    ...
    async def list(
        ...
        try:
            ...
            users_result = await self.db.execute(
                base_query.order_by(UserEntity.created_at.desc()).offset(skip).limit(limit)
            )
    ...
    async def update(self, user_id: int, user_data: UserUpdate) -> Optional[UserDetailed]:
        """Atualizar usuário"""
        try:
    ...
    async def count(self) -> Dict[str, int]:
        ...
        try:
            total_query = select(func.count()).select_from(UserEntity).where(
                UserEntity.deleted_at.is_(None)
            )
            
            active_query = select(func.count()).select_from(UserEntity).where(
                and_(
                    UserEntity.deleted_at.is_(None),
            ...
            inactive_query = select(func.count()).select_from(UserEntity).where(
                and_(
                    UserEntity.deleted_at.is_(None),
