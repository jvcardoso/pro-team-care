...
class B2BBillingRepository:
    ...
    async def get_subscription_plan_by_id(self, plan_id: int) -> Optional[SubscriptionPlan]:
        """Buscar plano por ID"""
    ...
    async def update_subscription_plan(self, plan_id: int, update_data: dict) -> Optional[SubscriptionPlan]:
    ...
    async def create_company_subscription(self, subscription_data: dict) -> CompanySubscription:
        ...
        try:
            ...
            logger.info("Subscription criada com sucesso", subscription_id=subscription.id)
            return subscription
        except Exception as e:
            await self.db.rollback()
            logger.error("Erro ao criar subscription", error=str(e), error_type=type(e).__name__, subscription_data=subscription_data)
            raise e
    ...
    async def get_company_subscriptions_batch(self, company_ids: List[int]) -> List[Optional[CompanySubscription]]:
    ...
    async def update_company_subscription(self, subscription_id: int, update_data: dict) -> Optional[CompanySubscription]:
    ...
    async def create_proteamcare_invoice(self, invoice_data: dict) -> ProTeamCareInvoice:
    ...
    async def get_proteamcare_invoice_by_id(self, invoice_id: int) -> Optional[ProTeamCareInvoice]:
        """Buscar fatura por ID"""
        query = (
            select(ProTeamCareInvoice)
            .options(
                selectinload(ProTeamCareInvoice.company).selectinload(Company.people),
                selectinload(ProTeamCareInvoice.subscription).selectinload(CompanySubscription.plan)
            )
    ...
    async def update_proteamcare_invoice(self, invoice_id: int, update_data: dict) -> Optional[ProTeamCareInvoice]:
        """Atualizar fatura Pro Team Care"""
        try:
            query = select(ProTeamCareInvoice).where(ProTeamCareInvoice.id == invoice_id)
            result = await self.db.execute(query)
    ...
    async def get_company_invoices(
        ...
        query = (
            select(ProTeamCareInvoice)
            .options(selectinload(ProTeamCareInvoice.subscription).selectinload(CompanySubscription.plan))
    ...
    async def get_invoices_by_status(self, status: str, limit: int = 100) -> List[ProTeamCareInvoice]:
    ...
    async def generate_invoice_number(self, company_id: int, year: int, month: int) -> str:
        ...
        query = (
            select(func.count(ProTeamCareInvoice.id))
            .where(
                and_(
    ...
    async def get_companies_for_billing(self, target_day: int) -> List[CompanySubscription]:
