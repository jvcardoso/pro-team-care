...
async def get_executive_dashboard(
    ...
    start_date: Optional[date] = Query(
        None, description="Data início período (YYYY-MM-DD)"
    ),
...
async def get_limits_status(
    contract_id: int, service: ContractDashboardService = Depends(get_dashboard_service)
):
...
async def get_contract_alerts(
    ...
    try:
        alerts_data = await service.get_contract_alerts(
            contract_id=contract_id, days_ahead=days_ahead
        )
...
async def get_usage_trends(
    ...
    months_back: int = Query(
        6, ge=1, le=24, description="Meses históricos para análise"
    ),
    service: ContractDashboardService = Depends(get_dashboard_service),
):
    ...
    try:
        trends_data = await service.get_usage_trends(
            contract_id=contract_id, months_back=months_back
        )
...
async def compare_contracts(
    contract_ids: str = Query(
        ..., description="IDs dos contratos separados por vírgula (ex: 1,2,3)"
    ),
    start_date: Optional[date] = Query(None),
    end_date: Optional[date] = Query(None),
    service: ContractDashboardService = Depends(get_dashboard_service),
