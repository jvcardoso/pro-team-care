...
async def get_table_structure(
    ...
    try:
        ...
        columns_result = await db.execute(columns_query, {"schema": schema, "table_name": table_name})
        columns = []
        ...
        constraints_result = await db.execute(constraints_query, {"schema": schema, "table_name": table_name})
        constraints = []
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter estrutura da tabela: {str(e)}")
...
async def get_table_data(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter dados da tabela: {str(e)}")
...
@router.get("/relationships/{table_name}", summary="Relacionamentos da tabela")
async def get_table_relationships(
    table_name: str,
    schema: str = Query(default="master", description="Schema do banco"),
    db: AsyncSession = Depends(get_db)
):
    """Obtém relacionamentos (foreign keys) de uma tabela"""
    try:
        relationships_query = text("""
            ...
            ORDER BY relationship_type, foreign_table
        """)
        
        result = await db.execute(relationships_query, {"schema": schema, "table_name": table_name})
        relationships = []
