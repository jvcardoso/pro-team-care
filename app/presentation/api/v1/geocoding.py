    ...
    async def nominatim_geocode(self, address: str) -> dict:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                raise HTTPException(
                    status_code=408, detail="Timeout na consulta de geocoding"
                )
            except httpx.RequestError as e:
                logger.error("Erro de rede no Nominatim", error=str(e), address=address)
                raise HTTPException(
                    status_code=503, detail="Serviço de geocoding indisponível"
                )
...
async def geocode_address(request: GeocodingRequest):
    ...
    try:
        ...
        if not request.address or not request.address.strip():
            raise HTTPException(status_code=400, detail="Endereço não pode estar vazio")
        ...
        logger.error(
            "Erro inesperado no geocoding", error=str(e), address=request.address
        )
        raise HTTPException(
            status_code=500, detail="Erro interno no serviço de geocoding"
        )
...
async def geocoding_health():
    ...
    return {"status": "healthy", "service": "geocoding", "provider": "nominatim"}
