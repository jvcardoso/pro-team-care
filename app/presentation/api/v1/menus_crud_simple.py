...
async def update_menu_simple(
    ...
    try:
        ...
        logger.info(
            "Menu atualizado via CRUD simples", menu_id=menu_id, user_id=current_user.id
        )
        ...
        return {
            ...
            "menu": {
                ...
                "created_at": (
                    updated_menu.created_at.isoformat()
                    if updated_menu.created_at
                    else None
                ),
                "updated_at": (
                    updated_menu.updated_at.isoformat()
                    if updated_menu.updated_at
                    else None
                ),
            },
        }
...
async def create_menu_simple(
    ...
    try:
        ...
        result = await db.execute(
            check_slug_query, {"slug": menu_data.slug, "parent_id": menu_data.parent_id}
        )
        ...
        if menu_data.parent_id:
            level_query = text(
                "SELECT level FROM master.menus WHERE id = :parent_id AND deleted_at IS NULL"
            )
        ...
        logger.info(
            "Menu criado via CRUD simples", menu_id=new_menu_id, user_id=current_user.id
        )
...
async def delete_menu_simple(
    ...
    try:
        ...
        logger.info(
            "Menu excluído via CRUD simples", menu_id=menu_id, user_id=current_user.id
        )
