...
from app.infrastructure.repositories.hierarchical_user_repository import HierarchicalUserRepository
from app.domain.entities.user import User
from app.presentation.schemas.user_legacy import User as UserSchema
...
async def list_users_hierarchical(
    ...
    company_filter: Optional[int] = Query(None, description="Filtrar por ID da empresa"),
    establishment_filter: Optional[int] = Query(None, description="Filtrar por ID do estabelecimento"),
    hierarchy_filter: Optional[str] = Query(None, description="Filtrar por nível hierárquico"),
    ...
    try:
        ...
        requester_hierarchy = await hierarchical_repo.get_user_hierarchy_info(current_user.id)
        ...
        return {
            ...
            "page": (skip // limit) + 1,
            "per_page": limit,
            "requesting_user_hierarchy": requester_hierarchy.get('hierarchy_level') if requester_hierarchy else 'UNKNOWN',
            "requesting_user_contexts": {
                "companies": requester_hierarchy.get('accessible_companies', []) if requester_hierarchy else [],
                "establishments": requester_hierarchy.get('accessible_establishments', []) if requester_hierarchy else []
            }
        }
...
async def get_current_user_hierarchy_info(
    ...
    try:
        ...
        hierarchy_info = await hierarchical_repo.get_user_hierarchy_info(current_user.id)
        ...
        roles_permissions = await hierarchical_repo.get_user_roles_and_permissions(current_user.id)
        
        # Contar usuários acessíveis
        accessible_users = await hierarchical_repo._get_accessible_user_ids(current_user.id)
...
async def get_accessible_users_summary(
    ...
    try:
        ...
        for user in users:
            ...
            stats["by_hierarchy"][hierarchy] = stats["by_hierarchy"].get(hierarchy, 0) + 1
