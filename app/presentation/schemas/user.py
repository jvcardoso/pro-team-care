...
class PersonCreatePF(BaseModel):
    ...
    tax_id: str = Field(
        ..., min_length=11, max_length=14, description="CPF (apenas números)"
    )
    ...
    person_type: PersonType = Field(
        PersonType.PF, description="Tipo de pessoa (sempre PF)"
    )

    # Metadados opcionais
    description: Optional[str] = Field(
        None, max_length=2000, description="Observações/Descrição"
    )
...
class UserPasswordChange(BaseModel):
    ...
    current_password: Optional[str] = Field(
        None, description="Senha atual (obrigatória se não for admin)"
    )
