...
class CompanySubscriptionCreate(CompanySubscriptionBase):
    ...
    send_activation_email: bool = Field(default=True, description="Enviar email de ativação automaticamente")
    recipient_email: Optional[str] = Field(None, description="Email do responsável (se não informado, busca emails cadastrados)")
...
class CompanySubscriptionResponse(CompanySubscriptionBase):
    ...
    def extract_company_name(cls, v, values):
        return getattr(values.get('company'), 'name', None) if hasattr(values.get('company', {}), 'name') else v

    @validator('plan_name', pre=True, always=True)
    def extract_plan_name(cls, v, values):
        return getattr(values.get('plan'), 'name', None) if hasattr(values.get('plan', {}), 'name') else v

    @validator('plan_monthly_price', pre=True, always=True)
    def extract_plan_price(cls, v, values):
        return getattr(values.get('plan'), 'monthly_price', None) if hasattr(values.get('plan', {}), 'monthly_price') else v
...
class SaasInvoiceDetailed(SaasInvoiceResponse):
    ...
    def extract_company_email(cls, v, values):
        return getattr(values.get('company'), 'email', None) if hasattr(values.get('company', {}), 'email') else v

    @validator('company_tax_id', pre=True, always=True)
    def extract_company_tax_id(cls, v, values):
        return getattr(values.get('company'), 'tax_id', None) if hasattr(values.get('company', {}), 'tax_id') else v
    ...
    def extract_subscription_plan_name(cls, v, values):
        ...
        if subscription and hasattr(subscription, 'plan') and hasattr(subscription.plan, 'name'):
            return subscription.plan.name
        return v

    @validator('subscription_billing_day', pre=True, always=True)
    def extract_subscription_billing_day(cls, v, values):
        return getattr(values.get('subscription'), 'billing_day', None) if hasattr(values.get('subscription', {}), 'billing_day') else v
