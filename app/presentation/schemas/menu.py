    ...
    def validate_badge_color(cls, v):
        ...
        if v:
            valid_colors = [
                ...
                'info', 'light', 'dark', 'red', 'green', 'blue', 'yellow'
            ]
...
class MenuDetailedSchema(MenuListSchema):
    ...
    children: List['MenuListSchema'] = Field(default_factory=list, description="Menus filhos diretos")
    siblings: List['MenuListSchema'] = Field(default_factory=list, description="Menus no mesmo nível")
...
class MenuStatisticsSchema(BaseModel):
    ...
    most_used_icons: List[Dict[str, Any]] = Field(default_factory=list, description="Ícones mais utilizados")
    recent_activity: Dict[str, int] = Field(default_factory=dict, description="Atividade recente")
...
class MenuImportRequestSchema(BaseModel):
    ...
    mode: str = Field("create", description="Modo de importação (create, update, upsert)")
    validate_hierarchy: bool = Field(True, description="Validar hierarquia durante importação")
