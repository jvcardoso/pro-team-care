...
class GetCompanyUseCase:
    ...
    async def get_by_id(self, company_id: int, user_id: int) -> Optional[CompanyEntity]:
        ...
        """
        logger.info("Buscando empresa por ID", company_id=company_id, user_id=user_id)
        ...
        if company:
            ...
            logger.warning(
                "Empresa não encontrada", company_id=company_id, user_id=user_id
            )
    ...
    async def get_by_cnpj(self, cnpj: str, user_id: int) -> Optional[CompanyEntity]:
        ...
        Returns:
            CompanyEntity ou None se não encontrada
        """
        logger.info("Buscando empresa por CNPJ", cnpj=cnpj, user_id=user_id)
        ...
        if company:
            ...
            logger.warning(
                "Empresa não encontrada por CNPJ", cnpj=cnpj, user_id=user_id
            )

        return company

    async def search(
        self, params: CompanyQueryParams, user_id: int
    ) -> List[CompanyEntity]:
        """
