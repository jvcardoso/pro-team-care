...
class UpdateCompanyUseCase:
    ...
    async def update_by_id(
        ...
        logger.info(
            ...
            phones_count=len(command.company_data.phones) if command.company_data.phones else 0,
            emails_count=len(command.company_data.emails) if command.company_data.emails else 0,
            addresses_count=len(command.company_data.addresses) if command.company_data.addresses else 0
        )
        ...
        # Atualizar empresa
        updated_company = await self.company_repository.update(command.company_id, command.company_data)
