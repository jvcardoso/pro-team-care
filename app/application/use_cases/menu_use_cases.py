...
from app.application.dto.menu_dto import (
    MenuCreateDTO, MenuUpdateDTO, MenuTreeResponseDTO, MenuTreeItemDTO,
    ReorderRequestDTO, MenuSearchRequestDTO, MenuSearchResultDTO,
    MenuStatisticsDTO, BulkUpdateRequestDTO, MenuOperationResultDTO
)
...
class CreateMenuUseCase:
    ...
    async def execute(self, data: MenuCreateDTO, created_by: int) -> MenuEntity:
        ...
        try:
            ...
            self.logger.info("Menu criado com sucesso", 
                           menu_id=result.id,
                           name=result.name,
                           level=result.level,
                           created_by=created_by,
            ...
            self.logger.error("Erro interno na criação de menu", 
                            error=str(e), 
                            created_by=created_by)
    ...
    async def execute(self, menu_id: int, data: MenuUpdateDTO, updated_by: int) -> Optional[MenuEntity]:
        ...
        try:
            ...
            if data.slug and data.slug != existing.slug:
                ...
                existing_with_slug = await self.repository.get_by_slug(data.slug, parent_id)
            ...
            if data.parent_id is not None and data.parent_id != existing.parent_id:
                if not await self.repository.validate_hierarchy_change(menu_id, data.parent_id):
                    raise MenuHierarchyException(
                        "Alteração criaria loop na hierarquia"
                    )
            ...
            execution_time = time.time() - start_time
            self.logger.info("Menu atualizado com sucesso", 
                           menu_id=menu_id,
            ...
            self.logger.error("Erro interno na atualização de menu", 
                            menu_id=menu_id,
                            error=str(e), 
                            updated_by=updated_by)
            raise MenuUseCaseException(f"Erro interno: {str(e)}")
...
class DeleteMenuUseCase:
    ...
    async def execute(self, menu_id: int, deleted_by: int) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info("Menu excluído com sucesso", 
                               menu_id=menu_id,
                               name=menu.name,
                               deleted_by=deleted_by,
                               execution_time=f"{execution_time:.3f}s")
            else:
                self.logger.warning("Falha na exclusão do menu", 
                                  menu_id=menu_id,
                                  deleted_by=deleted_by)
            ...
            self.logger.error("Erro interno na exclusão de menu", 
                            menu_id=menu_id,
                            error=str(e), 
                            deleted_by=deleted_by)
            raise MenuUseCaseException(f"Erro interno: {str(e)}")
...
class GetMenuTreeUseCase:
    ...
    async def execute(self, 
                     ...
                     include_inactive: bool = False) -> MenuTreeResponseDTO:
        """
        ...
        try:
            ...
            self.logger.info("Árvore de menus carregada", 
                           user_id=user_id,
                           context_type=context_type,
                           total_menus=total_menus,
                           root_menus=len(tree),
            ...
            self.logger.error("Erro ao buscar árvore de menus",
                            user_id=user_id,
                            context_type=context_type,
                            error=str(e))
            raise MenuUseCaseException(f"Erro interno: {str(e)}")
    ...
    async def execute(self, 
        ...
        try:
            ...
            if len(sort_orders) != len(set(sort_orders)):
                raise MenuValidationException("Sort orders duplicados não são permitidos")
            ...
            if success:
                self.logger.info("Menus reordenados com sucesso", 
                               parent_id=parent_id,
                               count=len(menu_orders),
                               reordered_by=reordered_by,
                               execution_time=f"{execution_time:.3f}s")
            else:
                self.logger.warning("Falha na reordenação dos menus", 
                                  parent_id=parent_id,
                                  reordered_by=reordered_by)
            ...
            self.logger.error("Erro interno na reordenação de menus", 
                            parent_id=parent_id,
                            error=str(e), 
                            reordered_by=reordered_by)
            raise MenuUseCaseException(f"Erro interno: {str(e)}")
...
class SearchMenusUseCase:
    ...
    async def execute(self, request: MenuSearchRequestDTO) -> List[MenuSearchResultDTO]:
        ...
        try:
            ...
            self.logger.info("Busca de menus realizada", 
                           query=request.query,
            ...
            self.logger.error("Erro na busca de menus",
                            query=request.query,
                            error=str(e))
            raise MenuUseCaseException(f"Erro interno: {str(e)}")
    
    def _calculate_relevance_score(self, menu: MenuEntity, query: str) -> tuple[float, str]:
...
class GetMenuStatisticsUseCase:
    ...
    async def execute(self) -> MenuStatisticsDTO:
        ...
        try:
            ...
            self.logger.info("Estatísticas de menus obtidas", 
...
class BulkUpdateMenusUseCase:
    ...
    async def execute(self, request: BulkUpdateRequestDTO, updated_by: int) -> MenuOperationResultDTO:
        """
        ...
        try:
            ...
            for menu_id in request.menu_ids:
                ...
                elif menu.status == MenuStatus.INACTIVE and request.status == MenuStatus.INACTIVE:
            ...
            if errors:
                return MenuOperationResultDTO(
                    ...
                    errors=[{"field": "menu_ids", "message": error, "code": "NOT_FOUND"} for error in errors],
                    warnings=warnings
                )
            ...
            if success:
                self.logger.info("Atualização em lote realizada", 
            ...
            self.logger.error("Erro na atualização em lote", 
            ...
            return MenuOperationResultDTO(
                ...
                errors=[{"field": "system", "message": str(e), "code": "INTERNAL_ERROR"}],
                warnings=[]
            )