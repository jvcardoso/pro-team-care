...
from app.domain.entities.menu import MenuStatus, MenuType
...
class MenuDetailDTO(MenuListDTO):
    ...
    children: List["MenuListDTO"] = Field(
        default_factory=list, description="Menus filhos diretos"
    )
    siblings: List["MenuListDTO"] = Field(
...
class MenuStatisticsDTO(BaseModel):
    ...
    recent_activity: Dict[str, int] = Field(
        description="Atividade recente (últimos 7 dias)"
    )
