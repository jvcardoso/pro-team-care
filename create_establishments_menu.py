    ...
    async with AsyncSession(engine) as session:
        try:
            ...
            result = await session.execute(
                text("SELECT id FROM master.menus WHERE slug = 'estabelecimentos' AND parent_id IS NOT NULL")
            )
            ...
            result = await session.execute(
                text("SELECT id FROM master.menus WHERE slug = 'administracao' AND parent_id IS NULL")
            )
            ...
            if parent_id:
                result = await session.execute(
                    text("SELECT COALESCE(MAX(sort_order), 0) + 1 as next_sort FROM master.menus WHERE parent_id = :parent_id"),
                    {"parent_id": parent_id}
                )
            else:
                result = await session.execute(
