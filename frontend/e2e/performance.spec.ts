...
test.describe('Testes de Performance', () => {
  test('deve carregar menus dinâmicos dentro do tempo limite', async ({ page }) => {
  ...
  test('deve manter performance ao alternar entre usuários múltiplas vezes', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('access_token', 'mock-token');
      localStorage.setItem('user', JSON.stringify({ id: 1, name: 'Test User' }));
      localStorage.setItem('useDynamicMenus', 'true');
  ...
  test('deve ser eficiente em múltiplas navegações', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('access_token', 'mock-token');
      localStorage.setItem('user', JSON.stringify({ id: 2, is_system_admin: true }));
      localStorage.setItem('useDynamicMenus', 'true');
  ...
  test('deve ter boa performance em diferentes tamanhos de tela', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('access_token', 'mock-token');
      localStorage.setItem('user', JSON.stringify({ id: 2, is_system_admin: true }));
      localStorage.setItem('useDynamicMenus', 'true');
  ...
  test('deve lidar eficientemente com cache de menus', async ({ page }) => {
    ...
    console.log(`✅ Primeira carga: ${firstLoadTime}ms, Com cache: ${cachedLoadTime}ms`);
  });
});