...
export class AdminPage {
  ...
  constructor(page: Page) {
    ...
    this.sidebar = page.locator('[role="navigation"][aria-label="Menu principal"]');
    ...
    this.menuItems = page.locator('[data-testid="menu-item"], .group.flex.items-center.px-2.py-2');
  ...
  async goto() {
    ...
    if (await this.loginPage.isOnLoginPage()) {
      console.log('🔐 Redirecionado para login, fazendo login automaticamente...');
  ...
  async waitForMenusToLoad() {
    ...
    await this.page.waitForFunction(() => {
      ...
      const dynamicMenus = document.querySelectorAll('[data-testid="menu-item"]');
      const staticMenus = document.querySelectorAll('.group.flex.items-center.px-2.py-2');
  ...
  async clickMenu(menuName: string) {
    const menu = this.page.locator(`[data-testid="menu-item"][data-menu-name="${menuName}"]`);
    await menu.click();
  }
  ...
  async getSubMenuCount(parentMenuName: string): Promise<number> {
    const parentMenu = this.page.locator(`[data-testid="menu-item"][data-menu-name="${parentMenuName}"]`);
  ...
  async refreshMenus() {
    const refreshButton = this.page.locator('[data-testid="refresh-menus-button"]');
  ...
  async validateMenuStructure() {
    ...
    if (menuCount === 0) {
      ...
      const errorMsg = await this.hasErrorMessage() ? await this.getErrorMessage() : 'Nenhum erro visível';
      console.error('❌ Nenhum menu encontrado:', { menuCount, errorMsg, debugInfo });
    }
