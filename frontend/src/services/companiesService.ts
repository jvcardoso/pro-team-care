...
class CompaniesService {
  ...
  async create(data: CompanyCreate): Promise<CompanyDetailed> {
    const result = await post<CompanyDetailed, CompanyCreate>(
      this.basePath,
      data
    );
  ...
  async update(id: number, data: CompanyUpdate): Promise<CompanyDetailed> {
    const result = await put<CompanyDetailed, CompanyUpdate>(
      `${this.basePath}/${id}`,
      data
    );
  ...
  async validateCNPJ(
    cnpj: string
  ): Promise<{ exists: boolean; company?: CompanyList }> {
    const cleanCNPJ = cnpj.replace(/\D/g, "");
    try {
      const company = await get<CompanyList>(
        `${this.basePath}/validate/cnpj/${cleanCNPJ}`
      );
