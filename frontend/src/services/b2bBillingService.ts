...
export class B2BBillingService {
  ...
  static async getSubscriptionPlans(activeOnly: boolean = true): Promise<SubscriptionPlan[]> {
  ...
  static async createSubscriptionPlan(planData: Omit<SubscriptionPlan, 'id' | 'created_at' | 'updated_at'>): Promise<SubscriptionPlan> {
    const response = await api.post(`${BASE_URL}/plans`, planData);
    return response.data;
  }

  static async updateSubscriptionPlan(planId: number, planData: Partial<Omit<SubscriptionPlan, 'id' | 'created_at' | 'updated_at'>>): Promise<SubscriptionPlan> {
    const response = await api.put(`${BASE_URL}/plans/${planId}`, planData);
    return response.data;
  }

  static async deleteSubscriptionPlan(planId: number): Promise<{ success: boolean; message: string }> {
  ...
  static async createCompanySubscription(data: CreateSubscriptionRequest): Promise<CompanySubscription> {
    const response = await api.post(`${BASE_URL}/subscriptions`, data);
    return response.data;
  }

  static async getCompanySubscription(companyId: number): Promise<CompanySubscription | null> {
    try {
      const response = await api.get(`${BASE_URL}/subscriptions/company/${companyId}`);
      return response.data;
    } catch (error: any) {
   ...
   */
  static async getCompanySubscriptionsBatch(companyIds: number[]): Promise<(CompanySubscription | null)[]> {
  ...
  static async updateCompanySubscription(
    ...
    const response = await api.put(`${BASE_URL}/subscriptions/${subscriptionId}`, updateData);
    return response.data;
  }
  ...
  static async createManualInvoice(data: CreateInvoiceRequest): Promise<ProTeamCareInvoice> {
  ...
  static async getCompanyInvoices(
    ...
    const response = await api.get(`${BASE_URL}/invoices/company/${companyId}`, {
      params: { status_filter: status, limit },
    });
    return response.data;
  }

  static async getInvoicesByStatus(status: string, limit: number = 100): Promise<ProTeamCareInvoice[]> {
  ...
  static async createCheckoutSession(invoiceId: number): Promise<CheckoutSessionResponse> {
    const response = await api.post(`${BASE_URL}/invoices/${invoiceId}/checkout`);
    return response.data;
  }
  ...
  static async markInvoiceAsPaid(
    ...
    const response = await api.post(`${BASE_URL}/invoices/${invoiceId}/mark-paid`, paymentData);
    return response.data;
  }
  ...
  static async bulkGenerateInvoices(data: BulkInvoiceGenerationRequest): Promise<BulkInvoiceGenerationResponse> {
