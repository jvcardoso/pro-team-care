...
import { Invoice, InvoiceTableRow, InvoiceListParams, PaymentMethod, InvoiceStatus } from '../../types/billing.types';
...
interface InvoiceTableProps {
  ...
  onPaymentUpdate?: (invoiceId: number, method: PaymentMethod, reference?: string) => void;
  showActions?: boolean;
  className?: string;
}
    ...
    },
    {
      ...
      render: (value, row) => (
        <div className="text-sm">
          ...
          <div className="text-gray-500">até {billingService.formatDate(row.billing_period_end)}</div>
        </div>
      ),
    },
    {
    ...
    columns.push({
      ...
      render: (_, row) => (
        <div className="flex space-x-2">
          ...
          {row.status !== InvoiceStatus.PAGA && row.status !== InvoiceStatus.CANCELADA && (
            <button
    ...
    <div className={`bg-white shadow rounded-lg overflow-hidden ${className}`}>
              ...
              <tr key={invoice.id} className="hover:bg-gray-50">
                ...
                {columns.map((column) => (
                  <td key={column.key} className="px-6 py-4 whitespace-nowrap text-sm">
                    {column.render
                      ? column.render(invoice[column.key as keyof InvoiceTableRow], invoice)
      ...
      {invoices.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">Não há faturas para exibir com os filtros atuais.</p>
        </div>
      )}
    </div>
  );
