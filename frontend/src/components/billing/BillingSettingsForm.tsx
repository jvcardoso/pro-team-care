...
const BillingSettingsForm: React.FC<BillingSettingsFormProps> = ({
  ...
  const billingCycleOptions = [
    { value: BillingCycle.MONTHLY, label: 'Mensal', description: 'Cobrança todo mês' },
    { value: BillingCycle.QUARTERLY, label: 'Trimestral', description: 'Cobrança a cada 3 meses' },
    { value: BillingCycle.SEMI_ANNUAL, label: 'Semestral', description: 'Cobrança a cada 6 meses' },
    { value: BillingCycle.ANNUAL, label: 'Anual', description: 'Cobrança uma vez por ano' },
  ];
  ...
  const validateForm = (): boolean => {
    ...
    if (settings.grace_period_days < 0 || settings.grace_period_days > 90) {
      newErrors.grace_period_days = 'Período de carência deve estar entre 0 e 90 dias';
    }

    if (settings.late_fee_percentage && (settings.late_fee_percentage < 0 || settings.late_fee_percentage > 20)) {
  ...
  const calculateNextBillingDate = (cycle: BillingCycle, day: number): string => {
    const now = new Date();
    let nextDate: Date;
  ...
  useEffect(() => {
    const nextDate = calculateNextBillingDate(settings.billing_cycle, settings.billing_day);
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center space-x-3">
          <input
            ...
            onChange={(e) => updateSetting('is_billing_enabled', e.target.checked)}
            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
          />
          <div>
            <label htmlFor="is_billing_enabled" className="text-sm font-medium text-gray-900">
          ...
          <>
                ...
                {billingCycleOptions.map((option) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => updateSetting('billing_cycle', e.target.value as BillingCycle)}
                      className="sr-only"
                    />
                    <div className="flex">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateSetting('billing_day', parseInt(e.target.value) || 1)}
                  className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 ${
                    errors.billing_day ? 'border-red-300' : 'border-gray-300'
                  }`}
                />
                {errors.billing_day && (
                  <p className="mt-1 text-xs text-red-600">{errors.billing_day}</p>
                )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    inputMode="decimal"
                    defaultValue={settings.amount_per_cycle ?
                    ...
                    className={`w-full pl-8 pr-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 ${
                      errors.amount_per_cycle ? 'border-red-300' : 'border-gray-300'
                    }`}
                  />
                </div>
                {errors.amount_per_cycle && (
                  <p className="mt-1 text-xs text-red-600">{errors.amount_per_cycle}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateSetting('next_billing_date', e.target.value)}
                className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 ${
                  errors.next_billing_date ? 'border-red-300' : 'border-gray-300'
                }`}
              />
              {errors.next_billing_date && (
                <p className="mt-1 text-xs text-red-600">{errors.next_billing_date}</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 ${
                    errors.grace_period_days ? 'border-red-300' : 'border-gray-300'
                  }`}
                />
                {errors.grace_period_days && (
                  <p className="mt-1 text-xs text-red-600">{errors.grace_period_days}</p>
                )}
                <p className="mt-1 text-xs text-gray-500">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateSetting('late_fee_percentage', parseFloat(e.target.value) || undefined)}
                  className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 ${
                    errors.late_fee_percentage ? 'border-red-300' : 'border-gray-300'
                  }`}
                />
                {errors.late_fee_percentage && (
                  <p className="mt-1 text-xs text-red-600">{errors.late_fee_percentage}</p>
                )}
                <p className="mt-1 text-xs text-gray-500">
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-3">
                <input
                  ...
                  onChange={(e) => updateSetting('auto_generate_invoices', e.target.checked)}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                />
                <label htmlFor="auto_generate_invoices" className="text-sm text-gray-900">
              ...
              <div className="flex items-center space-x-3">
                <input
                  ...
                  onChange={(e) => updateSetting('send_email_notifications', e.target.checked)}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                />
                <label htmlFor="send_email_notifications" className="text-sm text-gray-900">
