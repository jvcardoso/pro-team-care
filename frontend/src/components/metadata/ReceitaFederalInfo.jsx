  ...
  return (
    <Card title={title} className={className}>
      <div className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {metadata.cnaes_secundarios.map((cnae, index) => (
                <div key={index} className="text-sm text-muted-foreground bg-white dark:bg-gray-800 p-3 rounded border">
        ...
        {showSituacao && (metadata.situacao || metadata.data_situacao || metadata.tipo) && (
        ...
        {showCapital && (metadata.capital_social || metadata.porte || (metadata.municipio && metadata.uf)) && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {metadata.capital_social && (
              <div>
                ...
                <p className="text-sm text-muted-foreground bg-white dark:bg-gray-800 p-2 rounded border">
                  R$ {parseFloat(metadata.capital_social).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </p>
              </div>
            )}
            {metadata.porte && (
        ...
        {showNaturezaJuridica && (metadata.natureza_juridica || metadata.ultima_atualizacao_rf) && (
