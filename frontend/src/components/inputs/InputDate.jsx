...
const InputDate = ({
  ...
  const validateInput = (inputValue) => {
    ...
    if (cleanValue && cleanValue.length === 8) {
      ...
      if (minDate) {
        ...
        if (date < minDateObj) {
          setIsValid(false);
          setValidationMessage(`Data mínima: ${minDateObj.toLocaleDateString('pt-BR')}`);
          return false;
        }
      }
      ...
      if (maxDate) {
        ...
        if (date > maxDateObj) {
          setIsValid(false);
          setValidationMessage(`Data máxima: ${maxDateObj.toLocaleDateString('pt-BR')}`);
          return false;
        }
      }
    }
  ...
  const getInputClasses = () => {
    ...
    } else if (isValid && formattedValue && showValidation && formattedValue.length === 10) {
  ...
  return (
    <div className="space-y-1">
      ...
      {/* Dicas de range */}
      {!displayError && !formattedValue && !isFocused && (minDate || maxDate) && (
        <p className="text-xs text-muted-foreground">
          {minDate && maxDate ? (
            `Data entre ${new Date(minDate).toLocaleDateString('pt-BR')} e ${new Date(maxDate).toLocaleDateString('pt-BR')}`
          ) : minDate ? (
