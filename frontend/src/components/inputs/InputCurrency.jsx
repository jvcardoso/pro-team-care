...
const InputCurrency = ({
  ...
  const getInputClasses = () => {
    const baseClasses =
      "w-full px-3 py-2 border rounded-md bg-input text-foreground focus:ring-2 focus:ring-ring focus:outline-none transition-colors text-right";
    ...
    <div className="space-y-1">
      ...
      {!displayError &&
        ...
        (minValue !== null || maxValue !== null) && (
          <p className="text-xs text-muted-foreground">
            {minValue !== null && maxValue !== null
              ? `Valor entre ${formatCurrency(
                  (minValue * 100).toString()
                )} e ${formatCurrency((maxValue * 100).toString())}`
              : minValue !== null
              ? `Valor mínimo: ${formatCurrency((minValue * 100).toString())}`
              : `Valor máximo: ${formatCurrency((maxValue * 100).toString())}`}
          </p>
