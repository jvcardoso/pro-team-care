...
import { 
  Sun, Moon, Menu, Bell, User, Settings, LogOut, Mail, 
...
const Header = ({ sidebarCollapsed, onToggleSidebar, breadcrumb, isMobile, sidebarOpen }) => {
  ...
  const notifications = [
    { id: 1, title: 'Nova mensagem', content: 'João Silva enviou uma mensagem', time: '2 min', unread: true },
    { id: 2, title: 'Alerta do sistema', content: 'Backup agendado para hoje às 22:00', time: '1h', unread: true },
    { id: 3, title: 'Atualização', content: 'Relatório mensal disponível', time: '3h', unread: false }
  ];

  const messages = [
    { id: 1, sender: 'Maria Santos', content: 'Relatório de pacientes está pronto', time: '5 min', avatar: 'MS' },
    { id: 2, sender: 'Dr. João Silva', content: 'Precisamos revisar os protocolos', time: '1h', avatar: 'JS' },
    { id: 3, sender: 'Ana Costa', content: 'Reunião confirmada para amanhã', time: '2h', avatar: 'AC' }
  ];
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="hidden lg:flex items-center space-x-3">
                ...
                <div className="hidden xl:block">
                  <h1 className="text-sm font-semibold text-gray-900 dark:text-white">Pro Team Care</h1>
                  <p className="text-xs text-gray-500 dark:text-gray-400">AdminLTE Style</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center space-x-2">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.map((notification) => (
                      <div key={notification.id} className="p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">{notification.time} atrás</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="relative" ref={messagesRef}>
              ...
              {messagesOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 animate-fade-in z-50">
                  <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100">Mensagens</h3>
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {messages.map((message) => (
                      <div key={message.id} className="p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                        <div className="flex items-start space-x-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{message.sender}</p>
                            <p className="text-sm text-gray-600 dark:text-gray-400 truncate">{message.content}</p>
                            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{message.time} atrás</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 animate-fade-in z-50">
                  <div className="p-3 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">admin@procare.com</p>
                  </div>
                  <div className="py-1">
