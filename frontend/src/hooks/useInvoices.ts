...
interface UseInvoicesActions {
  ...
  updateInvoiceStatus: (id: number, status: InvoiceStatus, paymentDetails?: PaymentDetails) => Promise<Invoice>;
...
export const useInvoices = (initialFilters: InvoiceListParams = {}): UseInvoicesReturn => {
  ...
  const fetchInvoices = useCallback(async (params: InvoiceListParams = {}) => {
    ...
    try {
      const response: InvoiceListResponse = await billingService.listInvoices(mergedParams);
      ...
      const errorMessage = error.response?.data?.detail || 'Erro ao carregar faturas';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const fetchInvoice = useCallback(async (id: number) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.detail || 'Erro ao carregar fatura';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const createInvoice = useCallback(async (data: InvoiceCreate): Promise<Invoice> => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    ...
    try {
      ...
      const errorMessage = error.response?.data?.detail || 'Erro ao criar fatura';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const updateInvoice = useCallback(async (id: number, data: InvoiceUpdate): Promise<Invoice> => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        invoices: prev.invoices.map(inv => inv.id === id ? { ...inv, ...invoice } : inv),
        selectedInvoice: prev.selectedInvoice?.id === id ? { ...prev.selectedInvoice, ...invoice } : prev.selectedInvoice,
        loading: false,
      }));

      return invoice;
    } catch (error: any) {
      const errorMessage = error.response?.data?.detail || 'Erro ao atualizar fatura';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const updateInvoiceStatus = useCallback(async (
    ...
    try {
      const invoice = await billingService.updateInvoiceStatus(id, status, paymentDetails);
      ...
      const errorMessage = error.response?.data?.detail || 'Erro ao atualizar status da fatura';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const deleteInvoice = useCallback(async (id: number): Promise<void> => {
    ...
    try {
      ...
      setState(prev => ({
        ...
        selectedInvoice: prev.selectedInvoice?.id === id ? null : prev.selectedInvoice,
        loading: false,
      }));
    } catch (error: any) {
      const errorMessage = error.response?.data?.detail || 'Erro ao excluir fatura';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const overdueInvoices = useMemo(() => {
    return state.invoices.filter(invoice => billingService.isInvoiceOverdue(invoice));
  }, [state.invoices]);
  ...
  const paidInvoices = useMemo(() => {
    return state.invoices.filter(invoice => invoice.status === InvoiceStatus.PAGA);
  }, [state.invoices]);
