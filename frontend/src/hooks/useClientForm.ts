...
export const useClientForm = ({ clientId, onSave, establishmentId, establishmentCode }: UseClientFormProps) => {
  ...
  useEffect(() => {
    if (establishmentId && !clientId) {
      console.log(`🏥 Estabelecimento pré-selecionado: ${establishmentId} (${establishmentCode})`);
      ...
      if (establishmentCode && autoGeneratedEstablishmentRef.current !== establishmentCode && (!formData.client_code || formData.client_code.trim() === "")) {
        try {
          ...
          console.log(`✨ Código de cliente sugerido automaticamente: ${suggestedCode}`);
