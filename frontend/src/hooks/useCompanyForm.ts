...
import { Phone, Email, Address, People, PersonType, PersonStatus, PhoneType, EmailType, AddressType } from '../types';
...
export const useCompanyForm = ({ companyId, onSave }: UseCompanyFormProps) => {
  ...
  const [showNumberConfirmation, setShowNumberConfirmation] = useState<boolean>(false);
  const [pendingAddresses, setPendingAddresses] = useState<FormData | null>(null);
  ...
  const loadCompany = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      console.log('🔍 loadCompany - dados recebidos:', {
        'company.metadata (root)': company.metadata,
        'company.company?.metadata': company.company?.metadata,
        'company.people?.metadata': company.people?.metadata,
        'company.people?.name': company.people?.name
      });
      
      // Garantir que metadados CNAE estejam disponíveis em company.metadata
      // A API retorna metadata diretamente no root da empresa
      const companyMetadata = company.metadata || company.company?.metadata || company.people?.metadata || {};
      
      console.log('🔍 loadCompany - companyMetadata final:', companyMetadata);
  ...
  const handleCompanyFound = async (companyData: any): Promise<void> => {
    try {
      ...
      const existingCompany = await companiesService.getCompanyByCNPJ(cleanCNPJ);

      if (existingCompany) {
        // CNPJ já existe - mostrar mensagem de erro
        toast.error('Este CNPJ já está cadastrado no sistema. Você pode editar a empresa existente.', {
          duration: 6000,
          icon: '⚠️'
        });
        return;
      ...
      if (error.response?.status === 404) {
        ...
        toast.error('Erro ao verificar CNPJ. Prosseguindo com preenchimento automático.', {
          duration: 4000,
          icon: '⚠️'
        });
        await fillData(companyData);
  ...
  const fillData = async (companyData: any): Promise<void> => {
    ...
    const basicFormData: FormData = {
      people: {
        ...
        municipal_registration: companyData.people?.municipal_registration || '',
      ...
      phones: companyData.phones && companyData.phones.length > 0 ? companyData.phones.map((phone: any) => ({
      ...
      emails: companyData.emails && companyData.emails.length > 0 ? companyData.emails.map((email: any) => ({
      ...
      addresses: companyData.addresses && companyData.addresses.length > 0 ? companyData.addresses.map((address: any) => ({
        street: address.street || '',
        number: (address.number && address.number !== 'S/N' && address.number !== 's/n') ? address.number : '',
    ...
    if (basicFormData.addresses && basicFormData.addresses.length > 0) {
      ...
      try {
        ...
        const enrichedAddresses = await addressEnrichmentService.enriquecerMultiplosEnderecos(basicFormData.addresses);
        ...
        const hasEnrichedData = enrichedAddresses.some((addr: any) =>
          addr.latitude && addr.longitude && addr.ibge_city_code
        );

        const hasViaCepData = enrichedAddresses.some((addr: any) =>
          addr.ibge_city_code || addr.gia_code || addr.siafi_code
        );

        if (hasEnrichedData) {
          toast.success('Endereços enriquecidos com coordenadas GPS e dados oficiais!', {
            duration: 4000,
            icon: '🗺️'
          });
        } else if (hasViaCepData) {
          toast.success('Endereços enriquecidos com dados oficiais do ViaCEP!', {
            duration: 4000,
            icon: '🏠'
          });
        } else {
          console.warn('Endereços não foram enriquecidos completamente');
          toast('Dados básicos carregados, mas houve problemas no enriquecimento.', {
            duration: 3000,
            icon: '⚠️'
          });
        }
        ...
        toast('Dados básicos carregados, mas houve erro no enriquecimento de endereços.', {
          duration: 4000,
          icon: '⚠️'
        });
      }
  ...
  const handleNumberConfirmation = async (confirmed: boolean): Promise<void> => {
          ...
          for (const [key, value] of Object.entries(obj)) {
            ...
            if (key === 'metadata' && typeof value === 'object' && value !== null) {
              ...
              const camposRF = [
                ...
                'porte', 'capital_social', 'natureza_juridica', 'situacao',
                'motivo_situacao', 'data_situacao', 'situacao_especial',
                'data_situacao_especial', 'tipo', 'efr', 'municipio', 'uf',
                'ultima_atualizacao_rf', 'receita_ws_data', 'consulta_data'
              ];
