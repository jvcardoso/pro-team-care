...
class TestJWTSecurity:
    ...
    def test_jwt_security_manipulation(self):
        ...
        try:
            payload = jwt.decode(modified_token, settings.secret_key, algorithms=[settings.algorithm])
...
class TestSecurityHeaders:
    ...
    def test_security_headers_present(self, client: TestClient):
        ...
        for header in expected_headers:
            assert header in response.headers, f"Security header {header} não encontrado"
    ...
    def test_security_headers_values(self, client: TestClient):
        ...
        for header, expected_value in expected_values.items():
            ...
            assert actual_value == expected_value, f"Header {header}: esperado '{expected_value}', obtido '{actual_value}'"
...
class TestErrorHandling:
    ...
    def test_error_response_structure(self, client: TestClient):
        ...
        if response.status_code == 404:
            ...
            sensitive_info = [
                "database", "sql", "exception", "traceback",
                "internal", "debug", "error:", "failed:"
            ]
