...
class TestPagBankService:
    ...
    def mock_settings(self):
        ...
        mock_settings.BASE_URL = 'http://localhost:8000'
        mock_settings.FRONTEND_URL = 'http://localhost:3000'
        with patch('app.infrastructure.services.pagbank_service.settings', mock_settings):
            yield

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    @patch('httpx.AsyncClient')
    async def test_create_subscription_plan_success(self, mock_client, pagbank_service, mock_settings):
    ...
    @pytest.mark.asyncio
    @patch('httpx.AsyncClient')
    async def test_create_subscription_plan_api_error(self, mock_client, pagbank_service, mock_settings):
    ...
    @pytest.mark.asyncio
    @patch('httpx.AsyncClient')
    async def test_create_customer_success(self, mock_client, pagbank_service, mock_settings):
    ...
    @pytest.mark.asyncio
    @patch('httpx.AsyncClient')
    async def test_create_subscription_success(self, mock_client, pagbank_service, mock_settings):
        ...
        result = await pagbank_service.create_subscription("PLAN_123", "CUST_123", card_data)
    ...
    @pytest.mark.asyncio
    @patch('httpx.AsyncClient')
    async def test_cancel_subscription_success(self, mock_client, pagbank_service, mock_settings):
    ...
    @pytest.mark.asyncio
    @patch('httpx.AsyncClient')
    async def test_create_checkout_session_success(self, mock_client, pagbank_service, mock_settings):
    ...
    @pytest.mark.asyncio
    @patch('httpx.AsyncClient')
    async def test_get_transaction_status_success(self, mock_client, pagbank_service, mock_settings):
    ...
    async def test_verify_webhook_signature_valid(self, pagbank_service, mock_settings):
        ...
        with patch.object(pagbank_service, 'webhook_secret', secret):
            ...
            with patch('hmac.new') as mock_hmac:
                ...
                result = await pagbank_service.verify_webhook_signature(payload, "sha256=some_hash")

                assert result is True

    @pytest.mark.asyncio
    async def test_verify_webhook_signature_invalid(self, pagbank_service, mock_settings):
        ...
        with patch.object(pagbank_service, 'webhook_secret', "test_secret"):
            with patch('hmac.new') as mock_hmac:
                ...
                result = await pagbank_service.verify_webhook_signature(payload, "sha256=different_hash")
    ...
    def test_map_pagbank_status_to_internal(self, pagbank_service):
        ...
        assert pagbank_service.map_pagbank_status_to_internal("ACTIVE", "recurrent") == "approved"
        assert pagbank_service.map_pagbank_status_to_internal("SUSPENDED", "recurrent") == "declined"
        assert pagbank_service.map_pagbank_status_to_internal("CANCELLED", "recurrent") == "cancelled"

        # Payment statuses
        assert pagbank_service.map_pagbank_status_to_internal("PAID", "checkout") == "approved"
        assert pagbank_service.map_pagbank_status_to_internal("DECLINED", "checkout") == "declined"
        assert pagbank_service.map_pagbank_status_to_internal("PENDING", "checkout") == "pending"

        # Unknown status defaults to pending
        assert pagbank_service.map_pagbank_status_to_internal("UNKNOWN", "checkout") == "pending"