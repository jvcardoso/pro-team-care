...
class DependencyChecker:
    ...
    def check_layer_dependencies(self, source_layer: str, forbidden_layers: list) -> list:
...
class TestCleanArchitectureRules:
    ...
    def test_no_framework_dependencies_in_domain(self, dependency_checker):
        ...
        forbidden_frameworks = [
            'pydantic', 'sqlalchemy', 'fastapi', 'starlette',
            'flask', 'django', 'celery', 'redis'
        ]
            ...
            for imp in imports:
                for framework in forbidden_frameworks:
                    if framework in imp.lower():
                        violations.append({
                            'file': str(file_path.relative_to(dependency_checker.project_root)),
                            'import': imp,
                            'framework': framework
                        })
    ...
    def test_infrastructure_can_depend_on_domain_and_application(self, dependency_checker):
        """
        ...
        real_violations = [
            ...
            if not any(ignore in v['file'] for ignore in ['test_', '__pycache__', '.pyc'])
        ]
                ...
                if 'declarative_base' in imp or 'DeclarativeBase' in imp:
                    ...
                    if 'infrastructure/orm' not in str(path_obj):
                        sqlalchemy_imports.append({
                            'file': str(path_obj.relative_to(dependency_checker.project_root)),
                            'import': imp
                        })
